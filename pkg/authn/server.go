...
func (p *Authenticator) ServeHTTP(ctx context.Context, w http.ResponseWriter, r *http.Request, rr *requests.Request) error {
	...
	// rr.Upstream.Cookies = p.Cookies
	//rr.Upstream.CookieNames
	if p.UI.Title != "" {
	...
	// p.logger.Debug("ZZZ", zap.Any("url", urlPath))
	...
	/*
		...
		?		opts["auth_credentials_found"] = false
				...
				// TODO(greenpau): remove the below line.
				// opts["cookie_names"] = []string{redirectToToken, p.TokenProvider.TokenName}
				cookieNames := []string{redirectToToken}
				cookieNames = append(cookieNames, p.TokenProvider.GetTokenNames()...)
				opts["cookie_names"] = cookieNames
				opts["token_provider"] = p.TokenProvider

				if p.UI.Title != "" {
					opts["ui_title"] = p.UI.Title
				}
	...
	/*
		...
		case strings.HasPrefix(urlPath, "saml"), strings.HasPrefix(urlPath, "x509"), strings.HasPrefix(urlPath, "oauth2"):
			...
			for _, backend := range p.Backends {
				...
				opts["request"] = r
				// r.SourceBasePath = p.AuthURLPath
